...
public class MainActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		toolbarview = (LinearLayout) this.getLayoutInflater().inflate(R.layout.toolbarview, bottommainbar, false);

		/* SET XML LAYOUT INFLATOR - statusbarview.xml (res/layout) */
		statusbarview = (LinearLayout) this.getLayoutInflater().inflate(R.layout.statusbarview, bottommainbar, false);

		/* SET XML LAYOUT INFLATOR - backgroundview.xml (res/layout) */
		backgroundview = (LinearLayout) this.getLayoutInflater().inflate(R.layout.backgroundview, bottommainbar, false);

		/* SET XML LAYOUT INFLATOR - descriptionview.xml (res/layout) */
		descriptionview = (LinearLayout) this.getLayoutInflater().inflate(R.layout.descriptionview, bottommainbar,
				false);

		/* SET XML LAYOUT INFLATOR - videosizeview.xml (res/layout) */
		videosizeview = (LinearLayout) this.getLayoutInflater().inflate(R.layout.videosizeview, bottommainbar, false);
        
        /**/
        backgroundlayout = (LinearLayout) this.getLayoutInflater().inflate(R.layout.backgroundlayout, background, false);
		...
		bottombackground = findViewById(R.id.bottombackground);				bottommainbar = findViewById(R.id.bottommainbar);
		...
		LinearLayout showhidestatusbar = (LinearLayout) statusbarview.findViewById(R.id.showhidestatusbar);

		// FIND VIEW BY ID FROM XML LAYOUT - backgroundview.xml (res/layout) ~~
		LinearLayout choosecolorbutton = (LinearLayout) backgroundview.findViewById(R.id.choosecolorbutton);
        LinearLayout addbackgroundvideobutton = (LinearLayout) backgroundview.findViewById(R.id.addbackgroundvideobutton);        

		// FIND VIEW BY ID FROM XML LAYOUT - descriptionview.xml (res/layout) ~~
		LinearLayout showhidedescriptionbutton = (LinearLayout) descriptionview
				.findViewById(R.id.showhidedescriptionbutton);
		...
		LinearLayout trimbarbackground = (LinearLayout) videosizeview.findViewById(R.id.trimbarbackground);
		...
		videosizeseekbar.getProgressDrawable().setColorFilter(Color.parseColor("#FFFFFF"), PorterDuff.Mode.SRC_IN);
		...
		TrimView.setListener(new TrimTimelineViewListener() {
			@Override
		...
		showhidetoolbar.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
		...
		showhide3dot.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
		...
		showhidestatusbar.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
		...
		choosecolorbutton.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				ColorPickerDialogBuilder.with(MainActivity.this).setTitle(R.string.color_chooser_dialog_title)
						.initialColor(0xFFFFFFFF).wheelType(ColorPickerView.WHEEL_TYPE.FLOWER).density(15)
        ...
        addbackgroundvideobutton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent BACKGROUNDVIDEO = new Intent(Intent.ACTION_PICK,
						android.provider.MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
		...
		showhidedescriptionbutton.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case REQ_CD_PICKPROFILEPHOTO:
			if (resultCode == Activity.RESULT_OK) {
				...
				Intent CROPPROFILEPHOTO = CropImage.activity(imageUri).setAspectRatio(1, 1).setFixAspectRatio(true)
						.setGuidelines(CropImageView.Guidelines.OFF).setCropShape(CropImageView.CropShape.OVAL)
						.setScaleType(CropImageView.ScaleType.FIT_CENTER).setAutoZoomEnabled(true)
		...
		settimer1 = new TimerTask() {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					...
					public void run() {
						if (VIDEOTRIMMED) {
							if (videoview1.getCurrentPosition() >= Double.parseDouble(ENDTIMEinmillivar)) {
								videoview1.pause();
								videoview1.seekTo(Integer.parseInt(STARTTIMEinmillivar));
								videoview1.start();
							}
	...
	public static class VideoFrameDecoder extends AsyncTask<Integer, Integer, Bitmap> {
		...
		protected Bitmap doInBackground(Integer... objects) {
			...
			try {
				bitmap = mediaMetadataRetriever.getFrameAtTime(utils.frameTimeOffset * frameNum * 1000,
	...
	public static class TrimTimelineView extends View {
		...
		public boolean isDragging() {
			if (pressedPlay || pressedLeft || pressedRight)
				return true;
			return false;
		}
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				if (playEnabled && playX - additionWidthPlay <= x && x <= playX + additionWidthPlay && y >= 0
						&& y <= getMeasuredHeight()) {
					pressedPlay = true;
					if (listener != null)
						pressDx = (int) (x - playX);
					invalidate();
					return true;
				} else if (startX - additionWidth <= x && x <= startX + additionWidth && y >= 0
						&& y <= getMeasuredHeight()) {
					pressedLeft = true;
					if (listener != null)
		...
		private void reloadFrames(int frameNum) {
			...
			currentTask = new VideoFrameDecoder(this, new VideoFrameDecoder.VideoFrameDecoderListener() {
				@Override
		...
		protected void onDraw(Canvas canvas) {
			...
			drawableLeft.setBounds(startX - dp(8), dp(4) + (dp(44) - dp(18)) / 2, startX + dp(2),
					(dp(44) - dp(18)) / 2 + dp(18 + 4));
			drawableLeft.draw(canvas);
			rect3.set(endX + dp(2), dp(4), endX + dp(12), end);
			canvas.drawRoundRect(rect3, radius, radius, paint);
			drawableRight.setBounds(endX + dp(2), dp(4) + (dp(44) - dp(18)) / 2, endX + dp(12),
					(dp(44) - dp(18)) / 2 + dp(18 + 4));
